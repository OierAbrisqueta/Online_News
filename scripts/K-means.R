...
# To know how many clusters will be the best to group the data we will compare
# the total within-cluster sum of squares and the average silhouette width 

# Calculate WSS for 1 to 20 clusters
wss <- 0
kmax <- 20
for (i in 1:kmax){
  km <- kmeans(PCs_final, centers = i, nstart = 20)
  wss[i] <- km$tot.withinss
}

# Plot the total within-cluster sum of squares from 1 to 20 clusters
plot(1:kmax, wss, type = "b", xlab = "Number of clusters", ylab = "Total within-cluster sum of squares")
# There is no evident elbow in the plot. However in cluster 4 the steepest drop
# at the begging begins to soften, and in cluster 10 the curve flattens out noticeably
# again, and from then on the decline is very gradual.

# We check again which is the best number of clusters, but in this case using
# the average silhouette width method
# (This takes about 5 minutes to run, so you have the result in the report)
fviz_nbclust(PCs_final, FUNcluster = kmeans, nstart = 20, method = "silhouette", k.max = 15)
# This plot reveals that the best option would be to use 4 clusters. 

# With both total within-cluster sum of squares and average silhouette width check
# we decide that 4 clusters will the best number of clusters.

# Once we decide to use 4 clusters, we apply k-means clustering to the data
k4 <- kmeans(PCs_final, centers=4, nstart=20)

# We are going to print k4 to have information about the clusters
k4

# We cans see how:
# Cluster 1, size 1318, a very small and extreme profile, with V5 and V6 means 
# far from the average.
# Cluster 2, size 9183, has all negative means, especially in V1 and V2. Profile
# consistently below the global average.
# Cluster 3, size 10384, stands out mainly in V2 and V3, the rest is close to 0
# Cluster 4, size 14515, stands out in V1 where the rest of the clusters have negative means

PCs_final <- as.data.frame(PCs_final)
...
ggplot(df, aes(x= V1, y = V2, colour = clus)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  theme_minimal()
# We can see how cluster 1 is very positive in PC1 dimension, while the rest 
# of the clusters are negative. In PC2 dimension we see cluster 1 and 2 
# positives, cluster 4 around 0 and cluster 4 quite negative. All of this
# agres with wha has been seen in the cluster means.
